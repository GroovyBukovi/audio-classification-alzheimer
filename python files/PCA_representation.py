...
df["dx_x"] = df["dx_x"].map({"Control": 0, "ProbableAD": 1})

# Select only numeric features (excluding target variable)
feature_columns = [col for col in df.columns if col not in ["File", "dx_x", "dx_y"]]
...
df_pca["Diagnosis"] = df["dx_x"].map({0: "Control", 1: "ProbableAD"})
